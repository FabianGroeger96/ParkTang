...
async function create_user(tx) {
  create_user(uuidv4(), tx.firstName, tx.lastName, tx.email);
}
...
async function create_parkspace(tx) {
  create_parkspace(uuidv4(), tx.user, tx.coordinates, tx.address, tx.price, tx.free_from, tx.free_to, tx.disabled);
}
...
async function create_car(tx) {
  create_car(uuidv4(), tx.user, tx.name, tx.model, tx.features);
}
...
async function parking_out(tx) {
    ...
    var balance_after_transaction = parseFloat(tx.car.owner.balance) - parking_price;
		...
		if (balance_after_transaction >= 0) {
			...
			tx.parkspace.owner.balance = (parseFloat(tx.parkspace.owner.balance) + parking_price).toString();
 ...
 * @param {org.example.basic.create_example_data} tx The transaction instance.
 * @transaction
 */
async function create_example_data(tx) {
  // create Users
  var max = create_user('1000', 'Max', 'Mustermann', 'max.mustermann@gmail.com');
  var jhon = create_user('2000', 'Jhon', 'Doe', 'jhon.doe@gmail.com');
  var peter = create_user('3000', 'Peter', 'Kneubuehler', 'peter.k@gmail.com');

  // create Parkspace
  var parkspace_jhon = create_parkspace('80808080806504', jhon, "45,45", "gugus street", 0.35, "09:00", "18:00");

  // create Cars
  var car_max = create_car('805d0302476504', max, 'family car', 'Hyundai', ['family car', 'very long']);
  var car_peter = create_car('805d0302724004', peter, 'drop top', 'Mercedes', ['very big']);
}
...
/*
  ...
  -----------------
*/
function create_user(id, firstName, lastName, email) {
  var factory = await getFactory;
  var registry_user = await getParticipantRegistry(namespace + ".User");
  ...
  try {
    ...
    throw new Error('Unexpected Error when creating a new user')
  }

  return user;
}

function create_parkspace(id, user, coordinates, address, price, free_from, free_to, disabled) {
	var factory = await getFactory();
  ...
  try {
    ...
    throw new Error('Unexpected Error when creating a new parkspace')
  }
...
function create_car(id, user, name, model, features) {
	var factory = await getFactory();
