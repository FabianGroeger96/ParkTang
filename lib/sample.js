...
async function create_user(tx) {
	...
	var user = await factory.newResource(namespace, "User", uuidv4());
...
async function create_parkspace(tx) {
	...
	var parkspace_status = await factory.newResource(namespace, "ParkSpaceStatus", uuidv4());
	parkspace_status.free_from = tx.free_from;
	parkspace_status.free_to = tx.free_to;
	parkspace_status.disabled = tx.disabled;
	await registry_parkspace_status.add(parkspace_status);

	var parkspace = await factory.newResource(namespace, "ParkSpace", uuidv4());
...
async function create_car(tx) {
	...
	var car = await factory.newResource(namespace, "Car", uuidv4());
	car.owner = tx.user;
	car.name = tx.name;
	car.model = tx.model;
...
async function create_example_data(tx) {
  ...
  // create Parkspace
  create_parkspace('80808080806504', jhon, "45,45", "gugus street", 0.35, "09:00", "18:00")
